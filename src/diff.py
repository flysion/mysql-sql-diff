...
for m_table_key in m_tables:
	...
	print s_field_sort, '---', m_field_sort
	...
	# CHANGE COLUMN
	s_i = -1
	while s_i < len(s_field_sort) - 1:
		s_i += 1
		s_field_key = s_field_sort[s_i]

		if not s_fields.has_key(s_field_key):
			continue
			
		s_field = s_fields[s_field_key]
		s_field_sql = str(s_field)
		
		m_i = m_field_sort.index(s_field_key)
		m_field_key = m_field_sort[m_i]
		m_field = m_fields[m_field_key]
		m_field_sql = str(m_field)
		"""
		s_prev_field_key = None
		m_prev_field_key = None
		
		if s_i > 0:
			s_prev_field_key = s_field_sort[s_i - 1]
			
		if m_i > 0:
			m_prev_field_key = m_field_sort[m_i - 1]
		"""
		if s_field_sql == m_field_sql and s_i == m_i:
			continue
		...
		if s_i != m_i:
			if m_i > 0:
				sql += " AFTER `" + m_field.name + "`"
			else:
				sql += " FIRST"
				
			s_field_sort.pop(s_i)
			s_field_sort.insert(m_i, s_field_key)
